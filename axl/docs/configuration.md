...
On Windows, the generated `node` binary is a Linux executable. Run it from WSL (for example with `wsl.exe bash -lc 'cd /mnt/c/Users/HP/Desktop/Cambrian/axl && ./node -config node-config.json'`) rather than PowerShell.
